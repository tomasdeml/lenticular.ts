...
describe('Path Lens', () => {
    it('does not mutate input object when setting new value', () => {
        const path = l.pathFromExpression((d: IData, i, j) => d.list.items[i].attributes[j]);
    ...
    describe('Lens from path without indexes', () => {
        ...
        it('can set value', () => {
            const path = l.pathFromExpression((d: IData, i, j) => d.list.items);
            const lens = l.lensFromPath(path);
...
describe('Path Parser', () => {
    it('can create path from an untyped arrow expression', () => {
        const path = l.pathFromExpression((d, i) => d[2].foo.bar[i].baz[5]);
    ...
    it('can create path from an untyped function expression', () => {
        const path = l.pathFromExpression(function (d, i) { return d[2].foo.bar[i].baz[5]; });
    ...
    it('can create path from an typed arrow expression', () => {
        const path = l.pathFromExpression((d: IData, i) => d.list.items[i].name);
